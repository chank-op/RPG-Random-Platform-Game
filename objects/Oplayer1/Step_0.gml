if (fish = true) && (keyboard_check_pressed(vk_up)) && (fish_jump = false)
{
	ysp = 0
	ysp -= 2
	fish_jump = true
...
if (place_meeting(x, y+1, osolid_1))
{
    ysp = 0
    if (keyboard_check (vk_up))
    {
...
{
	if (room = Room40) && (current_month = 12) && (current_day = 3)
	{
		room = Room_Christmas;
	}
	if (room = Room40) && (current_month = 12) && (current_day = 24)
	{
		room = Room_Christmas;
	}
	else if (room = Room40) && (current_month = 10) && (current_day = 31)
	{
		room = Room_Halloween;
	}
	else if (room = Room40) && (current_month = 4) && (current_day = 20)
	{
		room = Room_Easter;
	}
	else
	...
	if (sprite_index = sFish)
	{
		fish_exist = false
	}
	if (sprite_index = sFish_r)
	{
		fish_exist = false
	}
}
...
if (fish = true) && (keyboard_check(vk_left))
{
	sprite_index = sFish
}

if (fish = true) && (keyboard_check(vk_right))
{
	sprite_index = sFish_r
}
...
if (place_meeting(x, y, oSolid_clear))
{
	instance_destroy(osolid_1)
}